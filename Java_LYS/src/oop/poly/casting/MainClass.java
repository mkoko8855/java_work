...
//		  위 child c = new Child(); 가 아니라
		...
		//강제 형 변환을 사용 할 수 없다.
		//일단 부모 객체를 생성해보자
		Parent ppp = new Parent();  //자식타입에 부모객체란 불가능하다.
		//그리고 형 변환을 해보자.
//		Child c3 = (Child) ppp;  //이건 실제로 조심해야 한다. 이건 빨간줄이 긁히지 않았다.
								 //새로운 c3 변수 생성 후, ppp라는 부모 객체 주소값에 child 타입을 강제로 끌어 내리고 대입했다. 위와 굉장히 유사하지만 다르다.
