package oop.static_.field;

public class MainClass {

	public static void main(String[] args) {
		
		Count c1 = new Count();
		
		c1.a += 5;
		c1.b += 5;
		System.out.println("인스턴스 변수 c1.a의 값은? : " + c1.a);
		System.out.println("정적 변수 c1.b의 값은? : " + c1.b);
		
		//출력 결과 :
		//인스턴스 변수 c1.a의 값은? : 5
		//정적 변수 c1.b의 값은? : 5
		
		System.out.println("--------------------------------------------");
		
		Count c2 = new Count();
		c2.a += 7;
		c2.b += 7;
		System.out.println("인스턴스 변수 c2.a : " + c2.a);
		System.out.println("정적 변수 c2.b : " + c2.b);
		//출력 결과 :
		//인스턴스 변수 c2.a : 7
		//정적 변수 c2.b : 12
		
		
		System.out.println("--------------------------------------------");

		Count c3 = new Count();
		c3.a += 8;
		c3.b += 8;
		System.out.println("인스턴스 변수 c3.a : " + c3.a);
		System.out.println("정적 변수 c3.b : " + c3.b);
		System.out.println("정적 변수 c1.b : " + c1.b);
		System.out.println("정적 변수 c2.b : " + c2.b);
		System.out.println("정적 변수 c3.b : " + c3.b);
		//출력 결과 :
		//인스턴스 변수 c3.a : 8
		//정적 변수 c3.b : 20
		//정적 변수 c1.b : 20
		//정적 변수 c2.b : 20
		//정적 변수 c3.b : 20

		
		
		
		/*
		 - static이 붙은 데이터는 그 static 데이터가 선언된
		 클래스 이름으로 바로 참조 할 수 있다.
		 
		 일반 멤버변수와 혼동 할 가능성이 있기 때문에
		 주소값으로 접근 하지 않습니다.
		 */
		
		Count.b++; //하나올려
		Count.b++; //하나 더올려, 22가 찍힐 것이다.
		System.out.println("정적 변수 Count.b의 값은 : " + Count.b);
		//출력 결과 : 
		//정적 변수 Count.b의 값은 : 22
	}
}
