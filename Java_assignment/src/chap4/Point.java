...
public class Point {
	...
	public void inputPoints(List<Student> sList) {
		...
		/*
		 * 1. 학생 객체를 1개 생성합니다. 2. 학생 객체에 속성값을 설정하는 메서드들을 호출해야 합니다. 3. 정보 저장이 완료된 객체를
		 * 리스트(sList)에 추가해야 합니다. 4. 저장 완료 메세지를 호출하세요. ex) XXX님의 성적 정보가 정상적으로 입력되었습니다.
		 */
	...
	public void showAllPoints(List<Student> showAll) { // 학생 객체들이 들어있는 리스트가 와야죠?
		/*
...
//		}
//		System.out.println("반 평균은 " + (avg / sList.size()));
//	}

		int sum = 0;
		
		for(Student Stu : showAll) {
			Stu.outputStuInfo();
			sum += Stu.getTotal();
		}
		
		double avg = (double)sum/showAll.size();
	...
	public void modifyPoint(List<Student> students) {
		...
		for (int i = 0; i < students.size(); i++) {
			if (students.get(i).getStuId().equals(idx)) {
	...
	public void deletePoint(List<Student> students) {
		...
		for (int i = 0; i < students.size(); i++) {
			if (students.get(i).getStuId().equals(idx)) {
				System.out.println(idx + " 님의 정보를 삭제합니다. [ y / n ]");
