...
//			의 형태로 출력합니다. 점수와 순위의 간격은 적절하게 띄워 주시면 되겠습니다.
		...
		for(int i = 0; i<scores.length; i++) {
			System.out.println("당신의 점수는 어떤가요? ");
			int answer = sc.nextInt();
			scores[i] = answer; //답을 점수에다가 넣을 것
			rank[i] = 1; //비교할 것, 등수를 내림, [배열값 못들어감]
		}
		
		for(int i=0; i<scores.length; i++) { //점수 순위
			for(int j=0; j<scores.length; j++) {
				if(scores[i] < scores[j]) {  //점수를 먼저 입력한 순서대로 다음 점수보다 작으면 등수를 내림
					rank[i] += 1;
				}
			}
		}
